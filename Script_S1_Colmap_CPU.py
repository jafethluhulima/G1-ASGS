...
def resolve_colmap_path(colmap_path):
    ...
    local_install = Path.home() / "colmap-x64-windows-nocuda" / "COLMAP.bat"

    if colmap_path == "colmap" and local_install.exists():
        print(f"Using COLMAP installation: {local_install}")
        return str(local_install)

    raise FileNotFoundError(
        "COLMAP executable not found. Provide its location with --colmap_path."
    )
